...
impl SandboxService {
    ...
    pub async fn create_sandbox(
        ...
        let record = SandboxRecord {
            ...
            secret_fixture_set: req.secret_fixture_set,
    ...
    pub async fn deploy_revision(
        ...
        let workspace = resolve_workspace(req.workspace_path.as_deref())?;
        ...
        let fidelity = build_fidelity(&record, &req, &rendered.render_path, &apply.image_refs);
        ...
        self.registry
            .update(sandbox_id, |r| {
                ...
                r.image_refs = apply.image_refs.clone();
                ...
                r.artifacts.push(ArtifactRecord {
                    id: artifact_id.clone(),
                    kind: "manifest".into(),
                    content: rendered.yaml.clone(),
                    created_at: now,
                });
            })
        ...
        Ok(DeployRevisionResponse {
            ...
            rendered_artifact_ids: vec![artifact_id],
            image_refs: apply.image_refs,
            fidelity,
    ...
    pub async fn observe_failure(
        ...
        let artifact_ids: Vec<String> = signature
            .evidence_refs
            .iter()
            .map(|e| e.id.clone())
            .collect();
    ...
    pub async fn run_validation(
        ...
        let results = validate::run_validation(
            self.backend.as_ref(),
            sandbox_id,
            &record.namespace,
            workspace.as_deref(),
...
fn resolve_workspace(path: Option<&str>) -> Result<String, DomainError> {
    if let Some(p) = path {
    ...
    std::env::var("RAPHAEL_DEFAULT_WORKSPACE").map_err(|_| {
        DomainError::InvalidRequest(
            "workspace_path required (or set RAPHAEL_DEFAULT_WORKSPACE)".into(),
        )
    })
}
